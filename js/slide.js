$(document).ready(function(){
    ...
    function smr_animation(){
        $(".slide_container>div>h2").css("animation","movement 1s ease-in-out forwards");
        $(".slide_container>div>p").css("animation","movement 1.3s ease-in-out forwards");
        $(".slide_container>div>div").css("animation","movement 1.6s ease-in-out forwards");
    }
    // 슬라이드 텍스트 요소 애니메이션 부여 함수

    function slide_select(){
        $(".slide_selector_list>li").eq(slide_num - 1).css("background-color","rgb(17, 17, 17)");
    }
    // 슬라이드 항목 선택시 선택한 항목 배경색 변경 함수

    function slide_smr(){
        $(".slide_container>div").css("display","none");
        $(".slide_container>div").eq(slide_num - 1).css("display","block");
    }
    // 슬라이드 텍스트 상자 변경 함수

    function bullet_select(){
        $(".bullet img").attr("src","./images/icon/state_out.png");
        $(".bullet img").eq(slide_num - 1).attr("src","./images/icon/state_over.png");
    }
    // 슬라이드 표시 그림 변경 함수
    ...
    $(".next_btn").click(function(){
        ...
        $(".slide_selector_list>li").eq(slide_num - 1).css("background-color","rgb(17, 17, 17)");
        smr_animation();
        $(".slide_selector_list>li").eq(slide_num - 2).css("background-color","rgb(47, 47, 47)");
        bullet_select();
    });
    // 슬라이드 다음 버튼
    ...
    $(".prev_btn").click(function(){
        ...
        smr_animation();
        $(".slide_selector_list>li").eq(slide_num - 2).css("background-color","rgb(47, 47, 47)");
        $(".slide_selector_list>li").eq(slide_num - 3).css("background-color","rgb(47, 47, 47)");
        $(".slide_selector_list>li").eq(slide_num - 4).css("background-color","rgb(47, 47, 47)");
        $(".slide_selector_list>li").eq(slide_num - 5).css("background-color","rgb(47, 47, 47)");
        bullet_select();
    });
    // 슬라이드 이전 버튼
